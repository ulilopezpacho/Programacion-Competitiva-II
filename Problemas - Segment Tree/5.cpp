...
int main() {
  ...
  for (auto &q : queries) {
    ...
    if (type == true) {
      int la = int(lower_bound(possibleSalaryValues.begin(), possibleSalaryValues.end(), a) - possibleSalaryValues.begin());
      int lb = int(upper_bound(possibleSalaryValues.begin(), possibleSalaryValues.end(), b) - possibleSalaryValues.begin());
      if (lb > 0) lb--;
