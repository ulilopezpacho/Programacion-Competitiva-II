...
struct SegmentTree {
  ...
  tint retrieve(Node &curr, tint a, tint b, tint k, tint x, tint y) {
    ...
    tint temp = 0;
    if (curr.left != nullptr) temp += retrieve(*curr.left,a,b,2*k,x,d);
    if (curr.right != nullptr) temp += retrieve(*curr.right,a,b,2*k+1,d+1,y);
    return temp;
  }

  tint retrieve(tint a, tint b) {
    return retrieve(*root, a, b, 1, 0, n-1);
  }
